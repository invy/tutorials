...
static EGLSetup *g_setup = nullptr;
...
static struct xdg_surface_interface my_xdg_surface_interface = {
    ...
    &xdg_surface_move, &xdg_surface_resize,
...
static struct wl_keyboard_interface keyboard_interface = {
    &keyboard_release
};
...
static struct wl_seat_interface seat_interface = {
    &seat_get_pointer,
    &seat_get_keyboard,
    &seat_get_touch
};
...
static Surface *cursor = nullptr;
static Surface *moving_surface = nullptr;
static Surface *active_surface = nullptr;
static Surface *pointer_surface = nullptr; // surface under the pointer
...
static void deactivate_surface (Surface *surface)
{
    ...
    {
        wl_keyboard_send_leave (surface->client->keyboard, 0, surface->surface);
    }
...
static void activate_surface (Surface *surface)
{
    ...
    int32_t *states = static_cast<int32_t*>(wl_array_add (&array, sizeof(int32_t)));
...
static void surface_commit (struct wl_client *client, struct wl_resource *resource)
{
    ...
    if (getSetup()->eglQueryWaylandBufferWL (g_backend->backend_get_egl_display(), surface->buffer, EGL_TEXTURE_FORMAT, &texture_format))
    {
		EGLint width, height;
        getSetup()->eglQueryWaylandBufferWL (g_backend->backend_get_egl_display(), surface->buffer, EGL_WIDTH, &width);
        getSetup()->eglQueryWaylandBufferWL (g_backend->backend_get_egl_display(), surface->buffer, EGL_WIDTH, &height);
		EGLAttrib attribs = EGL_NONE;
        EGLImage image = eglCreateImage (g_backend->backend_get_egl_display(), EGL_NO_CONTEXT, EGL_WAYLAND_BUFFER_WL, surface->buffer, &attribs);
        surface->texture.texture_delete ();
        surface->texture.texture_create_from_egl_image (width, height, image);
        eglDestroyImage (g_backend->backend_get_egl_display(), image);
	}
    else
    {
        ...
        surface->texture.texture_delete ();
        surface->texture.texture_create (width, height, data);
	}
...
{
    Surface *surface = static_cast<Surface*>(wl_resource_get_user_data (_surface));
...
static void seat_get_keyboard (struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
	...
	int fd, size;
    g_backend->backend_get_keymap (&fd, &size);
...
static void handle_mouse_motion_event (int x, int y)
{
    ...
    Surface *next_pointer_surface = NULL;
    Surface *s;
    wl_list_for_each_reverse (s, &surfaces, link)
    {
        ...
        if (x > s->x && y > s->y && x < s->x + s->texture.m_width && y < s->y + s->texture.m_height)
        {
    ...
    wl_pointer_send_motion (pointer_surface->client->pointer, g_backend->backend_get_timestamp(), surface_x, surface_y);
}
static void handle_mouse_button_event (int button, int state)
{
	...
	if (state == WL_POINTER_BUTTON_STATE_PRESSED) {
        if (pointer_surface != active_surface)
        {
            if (active_surface)
            {
                deactivate_surface (active_surface);
            }
			active_surface = pointer_surface;
            if (active_surface)
            {
                activate_surface (active_surface);
            }
		}
	}
    if (!pointer_surface || !pointer_surface->client->pointer)
    ...
    wl_pointer_send_button (pointer_surface->client->pointer, 0, g_backend->backend_get_timestamp(), button, state);
}
static void handle_key_event (int key, int state)
{
    ...
    wl_keyboard_send_key (active_surface->client->keyboard, 0, g_backend->backend_get_timestamp(), key, state);
}
static void handle_modifiers_changed (struct modifier_state new_state)
{
    if ( new_state.depressed == modifier_state.depressed &&
         new_state.latched == modifier_state.latched &&
         new_state.locked == modifier_state.locked &&
         new_state.group == modifier_state.group)
    {
        return;
    }
...
static void draw (void)
{
    ...
    wl_list_for_each_reverse (surface, &surfaces, link)
    {
        ...
        {
            surface->texture.texture_draw (pointer_x + surface->x, pointer_y + surface->y);
        }
		else
        {
            surface->texture.texture_draw (surface->x, surface->y);
        }
        if (surface->frame_callback)
        {
            wl_callback_send_done (surface->frame_callback, g_backend->backend_get_timestamp());
    ...
    if (cursor)
    {
        cursor->texture.texture_draw (pointer_x, pointer_y);
    }
	
	glFlush ();
    g_backend->backend_swap_buffers ();
}
...
static void main_loop (void)
{
    ...
    while (true)
    {
        ...
        {
            g_backend->backend_wait_for_events (wayland_fd);
		}
	}
}
...
int main (int argc, char **argv)
{
    ...
    g_backend->backend_init (&callbacks);

    auto eglBindWaylandDisplayWL = (PFNEGLBINDWAYLANDDISPLAYWL) eglGetProcAddress ("eglBindWaylandDisplayWL");
    auto eglQueryWaylandBufferWL = (PFNEGLQUERYWAYLANDBUFFERWL) eglGetProcAddress ("eglQueryWaylandBufferWL");
    auto glEGLImageTargetTexture2DOES = (PFNGLEGLIMAGETARGETTEXTURE2DOESPROC) eglGetProcAddress ("glEGLImageTargetTexture2DOES");

    printf("setting EGL up\n");
    g_setup = new EGLSetup(eglBindWaylandDisplayWL, eglQueryWaylandBufferWL, glEGLImageTargetTexture2DOES);

    wl_list_init (&clients);
	wl_list_init (&surfaces);

    printf("creating wwayland display\n");
    display = wl_display_create ();

    printf("adding socket (auto)\n");
    auto socket = wl_display_add_socket_auto (display);

    printf("socked: %s\n", socket);

    std::vector<struct wl_global *> wt_globals;
    wt_globals.push_back( wl_global_create (display, &wl_compositor_interface, wl_compositor_interface.version, NULL, &compositor_bind) );
    wt_globals.push_back( wl_global_create (display, &wl_shell_interface, wl_shell_interface.version, NULL, &shell_bind) );
    wt_globals.push_back( wl_global_create (display, &xdg_shell_interface, xdg_shell_interface.version, NULL, &xdg_shell_bind) );
    wt_globals.push_back( wl_global_create (display, &wl_seat_interface, wl_seat_interface.version, NULL, &seat_bind) );

    for(auto g : wt_globals)
    {
        if(g == nullptr)
        {
            fputs("error: wrong interface version", stderr);
            return 1;
        }
    }

    eglBindWaylandDisplayWL (g_backend->backend_get_egl_display(), display);
